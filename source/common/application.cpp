...
void GLAPIENTRY opengl_callback(GLenum source, GLenum type, GLuint id, GLenum severity, GLsizei length, const GLchar* message, const void* userParam)
{
    ...
    //std::cout << "OpenGL Debug Message " << id << " (type: " << _type << ") of " << _severit<< " raised from " << _source << ": " << message << std::endl;
}
...
int our::Application::run(int run_for_frames) {
    ...
    while(!glfwWindowShouldClose(window)){
         ...
         if(currentState == states["gameover-scene"])
        {
            ...
            ImGui::Begin("Game Over",0, ImGuiWindowFlags_NoBackground | ImGuiWindowFlags_NoMove| ImGuiWindowFlags_NoDecoration);
        ...
        //    ImGui::Begin("Main Menu");
            ImGui::Begin("Main Menu",0, ImGuiWindowFlags_NoBackground | ImGuiWindowFlags_NoMove| ImGuiWindowFlags_NoDecoration);
            ImGui::SetWindowSize(ImVec2(win_config.size.x,win_config.size.y));
            ImGui::PushFont(font2);
